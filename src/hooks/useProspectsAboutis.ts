...
import { getProspectsAboutis} from "@/lib/api"
...
export function useProspectsAboutis(initialFilter?: ProspectFilter): UseProspectsReturn {
  const { user } = useAuth();
  ...
  const fetchProspects = useCallback(async (filter?: ProspectFilter) => {
    ...
    try {
      const data = await getProspectsAboutis(filter);
      setProspects(data);
    } catch (err: any) {
