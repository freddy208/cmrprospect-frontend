...
import { getProspectsEntreprises} from "@/lib/api"
...
export function useProspectsEntreprises(initialFilter?: ProspectFilter): UseProspectsReturn {
  const { user } = useAuth();
  ...
  const fetchProspects = useCallback(async (filter?: ProspectFilter) => {
    ...
    try {
      const data = await getProspectsEntreprises(filter);
      setProspects(data);
    } catch (err: any) {
